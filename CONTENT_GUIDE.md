...
- **vocabulary**: Array von **20** `{ word, translation, example, mnemonic, category }` — `category` = Wochenthema in 1-2 Worten, `mnemonic` auf Deutsch. **Seit 2026-08-14: 20 Wörter pro Tag/Kapitel** (vorher 8, auf Nutzerwunsch angehoben, damit der Vokabeltrainer pro Kapitel mehr Auswahl bietet). Nicht jedes der 20 Wörter muss zusätzlich in `vocabPractice`/`quiz` vorkommen — diese bleiben bei ihrer bisherigen Zielgröße und decken eine sinnvolle Auswahl der wichtigsten/schwierigsten neuen Wörter ab, nicht zwingend alle 20.
- **vocabPractice**: Array von **24 Items** (choice/gap). Davon ca. 16-18 zu den neuen Wörtern des Tages, der Rest (**ab Tag 2**) gezielte Wiederholung von Vokabeln aus vorherigen Tagen derselben Woche (kumulatives Interleaving — siehe unten).
